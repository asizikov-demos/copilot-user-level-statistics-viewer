...
import type { ModelVendorTable } from '../read-models/models';
import { formatNumber } from '../utils/formatters';
import { modelVendorColors } from './charts/utils/chartColors';
import { getModelIcon, getVendorIcon } from './icons/ModelIcons';
import DisclosureSection from './ui/DisclosureSection';
import MetricsTable, { TableColumn } from './ui/MetricsTable';

interface ModelVendorBreakdownProps {
...
export default function ModelVendorBreakdown({ vendorTables }: ModelVendorBreakdownProps) {
  if (vendorTables.length === 0) return null;

  return (
    <div className="bg-white rounded-md border border-[#d1d9e0] print:border-gray-300 print:break-inside-avoid">
      <div className="px-6 py-4 border-b border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900">Models by Vendor</h3>
        <p className="text-sm text-gray-600 mt-1">
          Expand a vendor to see its models. Share is the model&apos;s portion of all user-initiated
          interactions in the report.
        </p>
      </div>
      <div className="px-6 py-4 space-y-3">
        {vendorTables.map(table => {
          const VendorIcon = getVendorIcon(table.vendor);
          return (
            <DisclosureSection
              key={table.vendor}
                ...
                <>
                  <span className="flex items-center gap-2 min-w-0">
                    ...
                    <span
                      ...
                      style={{ backgroundColor: modelVendorColors[table.vendor].solid }}
                      aria-hidden="true"
                    />
                    <span className="text-sm font-semibold text-gray-900 truncate">{table.vendor}</span>
